  ...
  <body style="font-family:arial;" onload="windows:print()">
    <div>
      <table border="0"  width="100%" style="margin-top:-20px;">
        ...
        <tr>
          <td colspan="6" height="60">
            <div style="margin : auto; margin-top:0; left:0; right:0; top:0; bottom:0; width:260px; height:80px;">
              <?= $this->tag->image(['img/logo.jpg', 'alt' => 'logo', 'width' => '55px', 'height' => '60px', 'style' => 'padding:5px; float: left;']) ?>
        ...
        <tr>
          ...
          <td>:</td>
          <td colspan="3"><?= $this->Prints->tanggakHari($invoice->tanggal_mulai) ?></td>
        </tr>
        <tr>
          <td width="18%"></td>
          <td width="25%">Kembali, hari/tgl.</td>
          <td>:</td>
          <td colspan="3"><?= $this->Prints->tanggakHari($invoice->tanggal_kembali) ?></td>
        </tr>
        <tr>
          <td width="18%" valign="top"></td>
          <td width="25%" valign="top">Tujuan Wisata</td>
          <td valign="top">:</td>
          <td colspan="3"><?= $this->Prints->TujuanRegular($invoice->route, $invoice->lokasi) ?></td>
        </tr>
        <tr>
          <td width="18%"></td>
          <td width="25%">Nama Penyewa</td>
          <td>:</td>
          <td colspan="3"><?= Phalcon\Text::upper($invoice->nama) ?></td>
        </tr>
        <tr>
          <td width="18%"></td>
          <td width="25%">Driver</td>
          <td>:</td>
          <td colspan="3"><?= Phalcon\Text::upper($this->Prints->Driver($invoice->driver, 'driver')) ?></td>
        </tr>
        <tr>
          <td width="18%"></td>
          <td width="25%">Co. Driver</td>
          <td>:</td>
          <td colspan="3"><?= Phalcon\Text::upper($this->Prints->Driver($invoice->co_driver, 'coDriver')) ?></td>
        </tr>
      </table>
      <!-- tabel bawah -->
      <br>
      <table border="1" style="border-collapse:collapse; border:1px black solid" width="100%">
        ...
        <?php $no = 1; ?>
        <?php $jumlah = 0; ?>
        <?php foreach ($this->Prints->Cost($invoice->kode) as $x) { ?>
        <tr height="38">
          <td style="padding:10px;"><?= $no ?>.</td>
          <td style="padding:10px;"><?= $x->cost ?></td>
          <td style="padding:10px;"><?= $x->satuan ?> <?= $x->persen ?></td>
          <td style="padding:10px;"><?= $this->Helpers->number($x->harga_satuan) ?></td>
          <td style="padding:10px;"><?= $this->Helpers->number($x->jumlah) ?></td>
          <td></td>
        </tr>
        <?php $jumlah = $jumlah + $x->jumlah; ?>
        <?php $no = $no + 1; ?>
        <?php } ?>
        <tr height="38">
          <td style="padding:10px;" colspan="4">TOTAL</td>
          <td style="padding:10px;"><?= $this->Helpers->number($jumlah) ?></td>
          <td></td>
        </tr>
      </table>
    </div>
    ...
    <div style="font-size: 12px;">
      <table border="0" width="100%">
        ...
        <tr height="20">
          <td colspan="5">
            <div style="margin :0; left:0; right:0; top:0; bottom:0; width:230px; height:80px;">
              <?= $this->tag->image(['img/logo.jpg', 'alt' => 'logo', 'width' => '55px', 'height' => '60px', 'style' => 'float: left;']) ?>
        ...
        <!-- isian data -->
        <?php $s = $this->Helpers->setting('spt'); ?>
        <tr>
        ...
        <tr>
          ...
          <td>Nama Driver</td>
          <td>:</td>
          <td colspan="2"> <?= Phalcon\Text::upper($this->Prints->Driver($invoice->driver, 'driver')) ?></td>
        </tr>
        <tr>
          <td></td>
          <td>Nama Co. Driver</td>
          <td>:</td>
          <td colspan="2"><?= Phalcon\Text::upper($this->Prints->Driver($invoice->co_driver, 'coDriver')) ?></td>
        </tr>
        <tr>
          <td></td>
          <td>No. Pol</td>
          <td>:</td>
          <td colspan="2"><?= $this->Prints->Bus($invoice->bus)->nomor_polisi ?></td>
        </tr>
        <tr>
          <td></td>
          <td>Jenis Bus</td>
          <td>:</td>
          <td colspan="2"><?= $this->Prints->Bus($invoice->bus)->ukuran ?></td>
        </tr>
        ...
        <tr>
          ...
          <td>:</td>
          <td colspan="2"><?= $this->Prints->tanggakHari($invoice->tanggal_mulai) ?></td>
        </tr>
        <tr>
          <td></td>
          <td>Pulang, hari/tgl.</td>
          <td>:</td>
          <td colspan="2"><?= $this->Prints->tanggakHari($invoice->tanggal_kembali) ?></td>
        </tr>
        <tr>
          <td></td>
        ...
        <tr>
          ...
          <td>Tujuan Wisata</td>
          <td>:</td>
          <td colspan="2"><?= $this->Prints->TujuanRegular($invoice->route, $invoice->lokasi) ?></td>
        </tr>
            ...
            <table>
              ...
              <td>
                <div style="border-bottom:1px solid black; width:150px; height:16px;" align="center">
                  <?= Phalcon\Text::upper($this->Prints->Driver($invoice->driver, 'driver')) ?>
                </div>
              </td>
              <td>)</td>
              </tr>
