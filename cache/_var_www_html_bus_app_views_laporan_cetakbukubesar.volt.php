...
<body style="font-family:Tahoma, Geneva, Verdana, sans-serif; font-size: 10px;" onload="windows:print()">
  ...
  <div>
    ...
    <table style="border-collapse: collapse;" width="100%" id="table" cellpadding="5">
      ...
      <?php $total_debet = 0; ?>
      <?php $total_kredit = 0; ?>
      <?php foreach ($jurnalChild as $j) { ?>
        <?php foreach ($bukuBesar as $x) { ?>
          ...
          <?php $total_debet = $total_debet + $j->debet; ?>
          <?php $total_kredit = $total_kredit + $j->kredit; ?>
          <?php } ?>
        <?php } ?>
      <?php $no = $no + 1; ?>
      <?php } ?>
      <tr>
        ...
        <td><b>Rp.<span style="float:right;"><?= $this->Helpers->number($total_debet) ?> ,-</span></b></td>
        <td><b>Rp.<span style="float:right;"><?= $this->Helpers->number($total_kredit) ?> ,-</span></b></td>
      </tr>
    </table>
  </div>
